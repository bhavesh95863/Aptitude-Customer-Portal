<template lang="html">
  <form novalidate @submit.stop.prevent="submit" v-if="selectedEntry" id="request-form">
    ...
    <div v-for="(parameter, i) in selectedEntry.parameters" :key="i">
      ...
      <v-col class="pt-0 pb-0" v-if="(parameter.schema.type === 'string' || parameter.schema.type === 'integer' || parameter.schema.type === 'number') && !parameter.schema.enum">
      <v-text-field
      ...
      <v-col class="pt-0 pb-0" v-if="parameter.schema.enum">
        <v-select
      ...
      <v-col class="pt-0 pb-0" v-if="parameter.schema.type === 'array' && parameter.schema.items.enum">
        <v-select multiple
      ...
      <v-col class="pt-0 pb-0" v-model="currentRequest.params[parameter.name]" :md-input-placeholder="parameter.name" :md-input-type="parameter.schema.items.type" v-if="parameter.schema.type === 'array' && !parameter.schema.items.enum">
